...
# Local Imports
from .utils import graph_intersection, split_datasets_old, multi_build_edges

device = "cuda" if torch.cuda.is_available() else "cpu"


class MultiEmbeddingBase(LightningModule):
    ...
    def setup(self, stage):
        self.trainset, self.valset, self.testset = split_datasets_old(
            **self.hparams
        )
    ...
    def get_hinge_distance(self, spatials, e_spatial, y_cluster):
        ...
        for i in range(1, len(spatials)):
            ...
            d_2 = torch.sum((reference - neighbors) ** 2, dim=-1)
    ...
    def get_truth(self, batch, e_spatial, e_bidir):
        ...
        # e_spatial_easy_fake = torch.unique(e_spatial_easy_fake, dim=0)
    ...
    def shared_evaluation(self, batch, batch_idx, knn_radius, knn_num, log=False, stage = "val"):
        ...
        hinge, d = self.get_hinge_distance(spatials, e_spatial.to(self.device), y_cluster)
        ...
        if log:
            ...
            self.log_dict(
                {"val_loss@{}".format(knn_radius): loss, "eff@{}".format(knn_radius): eff, "pur@{}".format(knn_radius): pur, "current_lr": current_lr}
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in self.hparams["r_val"]:
            outputs = self.shared_evaluation(
                batch, batch_idx, i, 500, log=True
            )
