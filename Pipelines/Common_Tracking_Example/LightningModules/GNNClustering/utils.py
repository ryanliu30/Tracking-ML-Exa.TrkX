...
class ClusteringDataset(Dataset):
    ...
    def __getitem__(self, key):
        ...
        event.pt[event.pt != event.pt] = 0
        cut_graph = event.graph[:, event.scores > self.hparams["score_cut"]]    
        graph = sp.sparse.coo_matrix((np.ones(cut_graph.cpu().shape[1]), cut_graph.cpu()), shape=(cut_graph.cpu().max()+1, cut_graph.cpu().max()+1)
                                    ).tocsr()
        _, labels = connected_components(graph, directed=False, return_labels=True)
        
        labels = torch.tensor(labels).long()
        
        event.assignment_init = nn.functional.one_hot(labels % self.hparams["clusters"], num_classes= self.hparams["clusters"]).float()
...
def generate_toys(num_tracks, num_layers, min_r, max_r, detector_width, ptcut, eff, pur, clusters):
    ...
    graph = sp.sparse.coo_matrix((np.ones(graph.cpu().shape[1]), graph.cpu()), shape=(len(node_feature), len(node_feature))
                                ).tocsr()
    _, labels = connected_components(graph, directed=False, return_labels=True)

    labels = torch.tensor(labels).long()

    event.assignment_init = torch.nn.functional.one_hot(labels % clusters, num_classes=clusters).float()
