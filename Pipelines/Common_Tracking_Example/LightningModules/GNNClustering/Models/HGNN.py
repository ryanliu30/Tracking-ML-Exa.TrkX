...
class SparseHierarchicalGNNBlock(nn.Module):
    def __init__(self, hparams):
        ...
        self.downward_network = make_mlp(
            3 * hparams["latent"],
            hparams["hidden"],
            hparams["latent"],
            hparams["nb_edge_layer"],
    ...
    def forward(self, nodes, edges, supernodes, superedges, upward_edges, downward_edges, graph, bipartite_graph, bipartite_graph_attention, super_graph, super_graph_attention):
        
        # Compute new supernode features
        node_messages = scatter_add(upward_edges*bipartite_graph_attention, bipartite_graph[1], dim=0, dim_size=bipartite_graph[1].max()+1)
        ...
        supernode_messages = scatter_add(downward_edges*bipartite_graph_attention, bipartite_graph[0], dim=0, dim_size=nodes.shape[0])
        ...
        # Compute new upward edge features
        upward_edge_input = torch.cat([nodes[bipartite_graph[0]], supernodes[bipartite_graph[1]], upward_edges], dim = -1)
        upward_edges = checkpoint(self.upward_network , upward_edge_input) + upward_edges
        
        # Compute new downward edge features
        downward_edge_input = torch.cat([supernodes[bipartite_graph[1]], nodes[bipartite_graph[0]], downward_edges], dim = -1)
        downward_edges = checkpoint(self.downward_network , downward_edge_input) + downward_edges
        
        self.upward_network
        
        return nodes, edges, supernodes, superedges, upward_edges, downward_edges
...
class SparseHierarchicalGNNAttention(nn.Module):
    def __init__(self, hparams, hdbscanmodel, logging):
        ...
        self.upward_edge_encoder = make_mlp(
            2*hparams["latent"],
            hparams["hidden"],
            hparams["latent"],
            hparams["nb_edge_layer"],
        ...
        self.downward_edge_encoder = make_mlp(
            2*hparams["latent"],
            hparams["hidden"],
            hparams["latent"],
            hparams["nb_edge_layer"],
    ...
    def forward(self, nodes, clusters):
        ...
        # Initialize Up & Downward edges
        upward_edges = checkpoint(self.upward_edge_encoder, torch.cat([nodes[bipartite_graph[0]], supernodes[bipartite_graph[1]]], dim=1))
        downward_edges = checkpoint(self.downward_edge_encoder, torch.cat([supernodes[bipartite_graph[1]], nodes[bipartite_graph[0]]], dim=1))
        
        return supernodes, superedges, upward_edges, downward_edges, bipartite_graph, bipartite_graph_attention, super_graph, super_graph_attention
...
class SparseHierarchicalGNN(EmbeddingBase):
    ...
    def forward(self, x, graph):
        ...
        supernodes, superedges, upward_edges, downward_edges, bipartite_graph, bipartite_graph_attention, super_graph, super_graph_attention = \
                    self.supergraph_construction(nodes, clusters)
        
        for layer in self.hgnn_blocks:
            
            nodes, edges, supernodes, superedges, upward_edges, downward_edges = layer(nodes,
                                                                                       edges,
                                                                                       supernodes,
                                                                                       superedges,
                                                                                       upward_edges,
