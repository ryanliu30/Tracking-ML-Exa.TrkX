...
class BC_HierarchicalGNN_HDBSCAN(BipartiteClassificationBase):
    ...
    def forward(self, x, graph):
        ...
        bipartite_scores = torch.sigmoid(
            checkpoint(self.output_layer, torch.cat([nodes[bipartite_graph[0]], supernodes[bipartite_graph[1]]], dim = 1))
        ).squeeze()
