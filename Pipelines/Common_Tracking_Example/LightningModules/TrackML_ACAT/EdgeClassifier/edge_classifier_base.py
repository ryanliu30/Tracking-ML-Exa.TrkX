...
class EdgeClassifierBase(LightningModule):
    ...
    def shared_evaluation(self, batch, batch_idx, log=False):
        ...
        prediction = self.GMM_model.fit_predict(scores.unsqueeze(1).cpu().numpy())
        cut = scores[prediction == self.GMM_model.means_.argmax()].min().item()
        
        cut = min(cut, self.hparams["score_cut"])
