...
class EdgeClassifierDataset(Dataset):
    ...
    def __getitem__(self, key):
        ...
        event.y = event.y.bool() & event.signal_mask[event.edge_index].all(0)
        event.y_pid = (event.pid[event.edge_index[0]] == event.pid[event.edge_index[1]]) & event.signal_mask[event.edge_index].all(0)
