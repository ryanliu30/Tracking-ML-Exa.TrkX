...
class ObjectCondensationBase(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        intermediate_embeddings, bipartite_edge_scores, bipartite_graph, supernodes = self(input_data, batch.graph)
    ...
    def shared_evaluation(self, batch, batch_idx, log=False):
        ...
        row_match, col_match, pt = cp.asarray(row_match), cp.asarray(col_match), cp.asarray(pt)
        maximum_scores, argmax = scatter_max(bipartite_edge_scores, bipartite_graph[0], dim=0, dim_size = bipartite_graph[0].max()+1)
        # selected_edges = argmax[(maximum_scores > self.hparams["noise_cut"]) & (argmax >= 0)].unique()
        ...
        n_particle_mask = (pid_object_mapping[row_match, col_match] > self.hparams["n_hits"])[0]
        row_match, col_match = row_match[n_particle_mask], col_match[n_particle_mask]
        
        majority_mask = (pid_object_mapping[row_match, col_match]/pid_object_mapping[:, col_match].sum(0) > self.hparams["majority_cut"])
        pt_mask = (pt[row_match] > self.hparams["ptcut"])
        
        track_eff = (majority_mask & pt_mask).sum()/(pt > self.hparams["ptcut"]).sum()
        track_pur = (pid_object_mapping[row_match, col_match]/pid_object_mapping[:, col_match].sum(0))[majority_mask].mean()
        
        fake_rate = 1 - (majority_mask & pt_mask).sum()/(pid_object_mapping.shape[1] - (pid_object_mapping.sum(0) < self.hparams["n_hits"]).sum() - (majority_mask & (pt[row_match] > 0) & (~pt_mask)).sum())
        particle_eff = pid_object_mapping[row_match, col_match][majority_mask & pt_mask].sum()/((batch.pt > self.hparams["ptcut"]) & (batch.pid != 0)).sum()
